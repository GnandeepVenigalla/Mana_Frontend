        ...
        <div className="page-container">
            <div className="page-header flex-stack" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
                <div>
                    ...
                    <p className="page-subtitle">AI-powered analysis and recommendations for {MONTHS[month - 1]} {year}</p>
                </div>
            ...
            {!data ? (
                <div className="empty-state card">
                    ...
                    <p className="empty-desc">We need more transaction data for this month to generate meaningful insights. Try uploading a statement or waiting until the end of the month.</p>
                </div>
            ) : (
                    ...
                    <div className="card flex-stack" style={{ display: 'flex', gap: 40, alignItems: 'center', textAlign: 'center', background: 'linear-gradient(145deg, var(--color-surface) 0%, rgba(30,41,59,0.5) 100%)' }}>
                        ...
                        <div style={{ flex: 1, width: '100%' }}>
                            <h2 className="text-center-mobile" style={{ fontSize: 24, fontWeight: 700, marginBottom: 8, color: '#fff' }}>
                                Your Financial Health is {scoreData?.spendingHealth ? scoreData.spendingHealth.charAt(0).toUpperCase() + scoreData.spendingHealth.slice(1) : 'Unknown'}
                            </h2>
                            <p style={{ color: 'var(--color-text-muted)', marginBottom: 20, lineHeight: 1.6 }}>
                                We calculated your score based on your savings rate, category spending limits, and overall net income for this month. You're currently saving <strong>{scoreData?.savingsRate}%</strong> of your income.
                            </p>
                            <div className="flex-stack" style={{ display: 'flex', gap: 12 }}>
                                ...
                                <div className="stat-card" style={{ flex: 1, padding: '16px 20px', background: 'rgba(0,0,0,0.2)' }}>
                                    ...
                                    <div className="stat-value" style={{ textTransform: 'capitalize' }}>{scoreData?.spendingHealth}</div>
                                </div>
                                {user?.creditScore > 0 && (
                                    <div className="stat-card" style={{ flex: 1, padding: '16px 20px', background: 'rgba(0,0,0,0.2)' }}>
                    ...
                    <div className="grid-2">
                        ...
                        <div className="card">
                            <h3 className="section-title mb-16" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <AlertTriangle size={18} color="#f59e0b" /> Alerts & Recommendations
                            </h3>
                            {insights.length > 0 ? (
                                ...
                                <p style={{ color: 'var(--color-text-muted)' }}>No special alerts for this month.</p>
                            )}
                        </div>
                        ...
                        <div className="card">
                            <h3 className="section-title mb-16" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <TrendingUp size={18} color="#8b5cf6" /> Investment Opportunities
                            </h3>
                            {investmentSuggestions && investmentSuggestions.length > 0 ? (
                                <div>
                                    <p style={{ color: 'var(--color-text-muted)', fontSize: 13, marginBottom: 16 }}>
                                        Based on your net savings, here are personalized investment ideas to grow your wealth.
                                    </p>
                                        ...
                                        {investmentSuggestions.map((inv, idx) => (
                                            <div key={idx} style={{ padding: 16, borderRadius: 12, border: '1px solid rgba(139,92,246,0.2)', background: 'linear-gradient(90deg, rgba(139,92,246,0.05) 0%, transparent 100%)' }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 12 }}>
                                                    <span style={{ color: 'rgba(255,255,255,0.8)' }}>Suggested allocation: <strong>${(inv.amount || 0).toFixed(0)}</strong></span>
                                ...
                                <div className="empty-state" style={{ padding: '40px 0' }}>
                                    <div className="empty-icon">🌱</div>
                                    <div className="empty-title">Increase Savings to Invest</div>
                                    <p className="empty-desc">We suggest having at least $100 in monthly net savings before exploring these investment options.</p>
                                </div>
                            )}
                        </div>
                    </div>
